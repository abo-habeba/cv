  ...
  <div v-if="userStore.user.theme">
    ...
    <v-dialog class="text-center" v-model="dialogItemForm" max-width="1200" persistent>
      <v-card class="pa-5">
        ...
        <div style="position: sticky; bottom: 0px; z-index: 100; background-color: blanchedalmond; width: 100%; max-width: 600px; margin: auto; border-radius: 7px">
...
const detailsTheme = ref(deepMerge(defaultDetailsTheme, props.detTheme));
newTheme.value = deepMerge(detailsTheme.value, props.detTheme);
onMounted(() => {
  if (userStore.user.theme) {
    newTheme.value = deepMerge(detailsTheme.value, userStore.user.theme[props.detTheme.nameEn]);
  } else {
    newTheme.value = deepMerge(detailsTheme.value, props.detTheme);
  }
});
...
watch(
  ...
  newThemeValue => {
    if (newThemeValue) {
      newTheme.value = deepMerge(detailsTheme.value, newThemeValue.theme[props.detTheme.nameEn]);
...
function resetMainTitle(notifySucces = true) {
  newTheme.value.MainTitle = deepMerge({}, defaultDetailsTheme.value.MainTitle);
  save();
  if (notifySucces) {
    notifySuccess('تم ارجاع التنسيقات بنجاح');
  }
...
function resetSubTitle(notifySucces = true) {
  newTheme.value.SubTitle = deepMerge({}, defaultDetailsTheme.value.SubTitle);
  save();
  if (notifySucces) {
    notifySuccess('تم ارجاع التنسيقات بنجاح');
  }
...
function resetParagraph(notifySucces = true) {
  newTheme.value.paragraph = deepMerge({}, defaultDetailsTheme.value.paragraph);
  save();
  if (notifySucces) {
    notifySuccess('تم ارجاع التنسيقات بنجاح');
  }
...
function resetButtons(notifySucces = true) {
  newTheme.value.Button = deepMerge({}, defaultDetailsTheme.value.button);
  save();
  if (notifySucces) {
    notifySuccess('تم ارجاع التنسيقات بنجاح');
  }
